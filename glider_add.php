...
<?php
	...
	include('functions.php');//Include the page where all the functions are located.
	$change_made = "";
	if (isset($_POST['save'])) {
			$reg = $_POST['reg'];
			...
			//testing
			//echo "$gregs", " ", "$gcallsign", " ", "$gtype", " ", "$gownership",  " ", "$auth_to_fly",  " ", "$ls1date", " ", "$gcost", " ", "$gcharge", " ", "$gnotes", "</br>";
			...
			elseif (checkExistance("tbl_gliders", "reg", $reg)){
			...
			elseif ($charge_calculation == ""){
				...
				if ($_SESSION['authorised'] == "Admin") {
					//Call the runQuery function to run the query to insert the glider record into the DB
					runQuery("INSERT INTO `tbl_gliders` (`reg`, `type_name`, `callsign`, `owner`, `auth_fly`, `ls1_exp`, `radio_exp_date`, `weight_bal_exp_date`, `hours`, `land` , `tot_launch` , `s_launch` , `w_launch` , `a_launch` , `m_launch` , `notes` , `charge_option` , `charge_calculation` , `active`) VALUES ('$reg', '$type', '$callsign', '$owner', '$auth_fly', '$ls1date', '$radio_exp', '$w_b_exp', '$hours' , 0 , '$launches' , 0 , 0 , 0 , 0 , '$notes', '$charge_option', '$charge_calculation', 'Y');");
					
					$session_message =  "Glider Successfully Added";
					$change_made = "Glider $reg added successfuly";
					//call function to add a change in the history table
					addHistory($_SESSION['mem_no'], $change_made, "Gliders");
					//After glider has been successfully added, clear fields in the form
					$reg = "";
					$callsign = "";
					$type = "";
					$owner = "";
		...
		// If the register button has been clicked show the names entered , Otherwise, show clean new fields.
			$reg = "";
			$callsign = "";
			$type = "";
			$owner = "";
...
?>
<p><strong>Adding a new glider:</strong></p>
<form id="addglider" name="addglider" method="post" action="glider_add.php">
