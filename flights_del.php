...
<?php
	...
	$delete_ID = $_GET['flight_no'];
	//Check to see if the user loggend in has the correct privelage to make the change
	if ($_SESSION['authorised'] == "Admin" || $_SESSION['authorised'] == "Finance" || $_SESSION['authorised'] == "Normal") {
				$deletearray = callQuery("SELECT * FROM tbl_flights WHERE flight_no = '$delete_ID'");
				runQuery("DELETE FROM tbl_flights WHERE flight_no = '$delete_ID'");
				echo "Record Deleted Successfully: " . " Flight No: " . $deletearray['flight_no'] . ". PIC (P1): " . $deletearray['p1'] . ". Take Off Time: " . $deletearray['take_off_time'];
				...
				if ($delaccountarray['balance'] != 0){
						echo "<strong>Member can not be removed. The Account Balance for this member is not R0.</strong>" ;
				}
				else {
					//Call the functions to execute queries to remove the member record and associated account record
					runQuery("DELETE FROM tbl_members WHERE mem_no = '$delete_ID'");
					runQuery("DELETE FROM tbl_accounts WHERE mem_no = '$delete_ID'");
					echo "Record Deleted Successfully: " . " Name: " . $deletearray['name'] . ". Surname: " . $deletearray['surname'] . ". Member Number: " . $deletearray['mem_no'] . ". Account Number: " . $delaccountarray['account_no'];
					$change_made = "Deleted Member Record and Account for member $delete_ID ".$deletearray['name']." ". $deletearray['surname'];
	...
	?>
<p align="left"><a href="flights.php">Flights</a></p>
